<?php
	...
	error_reporting(0);
	$content_search = trim(to_slug($_POST['content_search']), '-');
	//echo $content_search;
	$sql = "SELECT * FROM product WHERE status = 1 AND slug LIKE '%$content_search%'";
	...
	?>
		<div class="row">
					<div class="col my-5">
						<!-- Product Sorting -->
						<div class="pull-left">
							<p>Find <?php echo $numRow?> products</p>
						</div>
				...
				<ul class="pagination">
					...
					<?php
						for ($i = 1; $i <= $total_page; $i++){
							$pos = ($i - 1) * $limit;
					?>
						<li class="pagination-item <?php echo ($i==1) ? 'is-active' : '' ?>"> <a class="pagination-link" onclick="getProduct_ver2(<?php echo $pos ?>,<?php echo $i ?>,<?php echo $limit ?>,'%','<?php echo $content_search ?>')"><?php echo $i?></a> </li>
					<?php
						}
					?>
